...
/**
 * does a checkboard pattern
 *
 * @author Shrif Rai
 */
public class absolutezenbot extends Thread
{
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int j = 0; j < 5; j++) {
            if(j % 2 == 0) {
                absolutezenbot t1 = new absolutezenbot(0, j, 3, args[0], Integer.parseInt(args[1]), "t" + x);
                t1.start();
                threadList.add(t1);
            }
            else
            {
                absolutezenbot t1 = new absolutezenbot(0, j, 0, args[0], Integer.parseInt(args[1]), "t" + x);
                t1.start();
                threadList.add(t1);
            }
            x++;
    ...
    public absolutezenbot(int row, int col, int color, String host, int port, String username) {
        this.row = row;
        this.col = col;
        this.color = color;
        this.port = port;
